...
parser.add_argument('-i', '--emotibit_info', type=str, required=False, default='', help='EmotiBit info string from EmotiBit serial console used to specify hardware and firmware. NOTE: this strips the quotes out of the JSON. ToDO: add provision to preserve JSON quotes.')
...
testing_info['EmotiBitInfo'] = args.emotibit_info
print(testing_info['EmotiBitInfo'])

# Parse the data
dst_filebase = eb_utils.parse_data(src_path=args.src_path, dst_dir=args.dst_dir, parser_path = args.parser_path, dst_filebase = args.dst_filebase, ts_to_filebase = args.ts_to_filebase, rm_src = args.rm_src)
